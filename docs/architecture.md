...
and deletes. Graph relationships also form an id-ordered SHA-256 chain, which
is backfilled during migration and verified during investigation replay.
Enrichment snapshots now reject updates and deletes as well;
observation IDs are projected from the append-only link table into returned
decision traces instead of rewriting a saved snapshot. These chains detect
...
5. **Provider evaluation and scheduling:** a versioned fixture evaluator
   measures coverage, failure, latency, freshness, disagreement, overlap,
   classification errors, and successful no-data misses on known malicious
   cases. A Youden's J weight candidate is provided for operator review;
   evaluation never changes scoring weights automatically. The engine now uses
   a bounded single-process
...
consistently before evidence identity is calculated. Enrichment snapshots
...
first supplied spelling and source context for each identity. Relationship
endpoints are canonicalized before self-link checks, provenance matching, and
