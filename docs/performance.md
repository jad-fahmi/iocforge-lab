...
investigation linking, historical replay, one-hop pivot lookup, row growth, and
database growth above the initialized schema size. It prints a JSON report and never calls
external providers or writes to the user's history database.
...
The report includes a digest of the generated IOC set, methodology version,
runtime, platform, parameters, row counts, and measured timings. Timings are
...
measured 1,720.8 indicators/second without history persistence and 94.9
...
took 0.80 seconds. Replaying 20 saved snapshots had a 0.411 ms p50 and 0.568 ms
p95; one-hop pivot lookup took 0.966 ms. The database grew by 937,984 bytes,
about 9.4 KB per indicator for this workload.
